...
void main()
{
	...
	vs_particlePos.y = particlePos.y + vel.y * elapsedTime;
	vs_particlePos.y += (0.03)*particleColor.y * elapsedTime;
	vs_particlePos.x = (age+sqrt(age)*4)*sin(particleVel.w *4) + (age+sqrt(age)*4)*sin(particleVel.w *4 - particleColor.x) + (0.1+age)*particleColor.z + ori_position.x;
	vs_particlePos.z = (age+sqrt(age)*4)*cos(particleVel.w *4) + (age+sqrt(age)*4)*cos(particleVel.w *4 - particleColor.x) + (0.1+age)*particleColor.x + ori_position.z;
