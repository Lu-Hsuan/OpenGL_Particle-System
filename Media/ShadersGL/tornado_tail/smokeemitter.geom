...
void main()
{
	...
	if (age >= emitRate) {
		for (int i = 0; i < 80; ++i) {
			...
			vec3 pos;
			vel.x = 0;
			vel.z = -0.05 - 0.05*randomFloat(seed);
			vel.y = 0.2;

			my_out.particlePos.xyz	= vs_particlePos[0].xyz - vel*i*0.1;
			my_out.particlePos.w	= 1.0;
			my_out.particleVel.xyz	= vel;
			my_out.particleVel.w	= 0;
			my_out.particleColor	= vec4(randomScalar3(seed),i/80.0);
			EmitVertex();
			//seed += 4207.56;
		}
	}
