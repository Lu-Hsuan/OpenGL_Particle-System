...
void mapTexture(GLuint &texture, const char *name){
    ...
    stbi_uc *texData = stbi_load(name, &width, &height, &channels, STBI_default);
	if (texData == nullptr) {
		std::cout << "ERROR";
		exit;
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, channels == 4 ? GL_RGBA : GL_RGB, GL_UNSIGNED_BYTE, texData);
